...
@implementation FTWDevice

@synthesize name, localizedName, systemName, systemVersion, model, uniqueIdentifier;

@synthesize MACAddress, platform, hardwareModel;
@synthesize CPUFrequency, busFrequency, CPUCount, totalMemory, userMemory;
@synthesize totalDiskSpace, freeDiskSpace, isRetina, deviceFamily, serialNumber;
...
- (NSString*) MACAddress {
    ...
    if (KERN_SUCCESS != kernResult) {
        ...
        kernResult = GetMACAddress(intfIterator, lMACAddress, sizeof(MACAddress));
...
- (NSString*) uniqueIdentifier {
    return [[[self name] stringByAppendingString:[self MACAddress]] MD5Hash];
}
